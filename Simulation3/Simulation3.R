...
w <- 50 * (exp(-0.5*((18*(x_grid-0.2))^2 + (15*(y_grid-0.8))^2)) + exp(-0.5*((24*(x_grid-0.8))^2 + (18*(y_grid-0.2))^2))) # length q
z <- 0.1*(x_grid-0.5)^2 + (y_grid-0.5)^3 # runif(q, -0.5, 0.5) # length q
...
NN <- neighborhood(mu.bar) ; dim(NN) ; NN[1:3,]
# Propagator matrix
...
x <- runif(length(vec(u.all[])), -0.05, 0.05) ; class(x) ; summary(x) ; length(x)
...
chain1 <-fit.model.mcmc(
  ...
  alpha.start = alpha + runif(2, -0.001, 0.001),
  beta.start = beta + runif(1, -0.001, 0.001),
  gamma.start = gamma + runif(2, -0.001, 0.001),  
  theta.start = theta + runif(1, -0.01, 0.001),  
  phi.start = phi + runif(1, -0.01, 0.01), 
  alpha.prior.var = 10^6, beta.prior.var = 10^6, gamma.prior.var = 10^6,
  theta.prior = 10^6, phi.prior = 10^6,
  alpha.tune = c(1/150,1/500), beta.tune = 1/60, gamma.tune = c(0.5/10^5,1/1e5), theta.tune = 2, phi.tune = 1/500,
...
#### 8. cross entropy loss and accuracy ####
