...
w <- (exp((x_grid-0.5)^2 + (y_grid-0.5)^2)) #runif(q, -0.5, 0.5) # length q
...
chain1 <-fit.model.mcmc(
  ...
  alpha.start = alpha + rnorm(2, 0, sd = 0.1),
  beta.start = beta + rnorm(1,0,0.1),
  gamma.start = gamma + rnorm(2, 0, sd = 0.1),  
  theta.start = theta + rnorm(1,0,0.1),  
  phi.start = phi + rnorm(1, 0, 0.01), 
  alpha.prior.var = 10^6, beta.prior.var = 10^6, gamma.prior.var = 10^6,
  theta.prior = 10^6, phi.prior = 10^6,
  alpha.tune = c(1/300,1/2000), beta.tune = 1/30, gamma.tune = c(1/10^5,1/500), theta.tune = 3, phi.tune = 1/350,
